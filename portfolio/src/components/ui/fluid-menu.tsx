    ...
    return (
        <div className="relative w-[64px]" data-expanded={isExpanded}>
            ...
            <div className="relative">
                ...
                <div
                    className="relative w-16 h-16 bg-white dark:bg-zinc-800 cursor-pointer rounded-full group will-change-transform z-50 shadow-lg border border-zinc-200 dark:border-zinc-700"
                ...
                {childrenArray.slice(1).map((child, index) => (
                    <div
                        key={index}
                        className="absolute top-0 left-0 w-16 h-16 bg-white dark:bg-zinc-800 will-change-transform rounded-full shadow-lg border border-zinc-200 dark:border-zinc-700 flex items-center justify-center"
                        style={{
                            transform: `translateY(${isExpanded ? (index + 1) * 72 : 0}px)`,
                            opacity: isExpanded ? 1 : 0,
                            zIndex: 40 - index,
                            // Removed clip-path to show full circle for items
